...
async def run_batch(  # noqa: C901
    ...
    async def worker(index: int, assignment: TrustedAssignment) -> None:
        async with paper_limiter:
            try:
                if _load_cached(store, assignment) is not None:
                    item = BatchItemResult(
                        index, assignment.paper_id, BatchStatus.CACHED
                    )
                else:
            ...
            results[index] = item
            if item.status is BatchStatus.CACHED:
                return
    ...
    async with anyio.create_task_group() as task_group:
        for index, assignment in enumerate(assignments):
            _ = task_group.start_soon(worker, index, assignment)
    ...
    return BatchSummary(
        ...
        completed_count=sum(
            item.status in {BatchStatus.COMPLETED, BatchStatus.CACHED} for item in items
        ),
...
def _load_cached(
    ...
    path = store.paper_directory(assignment.paper_id) / "final_review.json"
    try:
        ...
        return None
    return submission if submission.paper_id == assignment.paper_id else None
